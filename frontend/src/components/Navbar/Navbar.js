...
const Navbar = () => {
  ...
  return(
    <AppBar className={classes.appBar} position="static" color="inherit">
       ...
       <Toolbar className={classes.toolbar}>   {/* if user is logged in or if user needs to log in */}
        {user?.result ? (
          <div className={classes.profile}>
            <Avatar className={classes.purple} alt={user?.result.name} src={user?.result.imageUrl}>{user?.result.name.charAt(0)}</Avatar>
            <Typography className={classes.userName} variant="h6">{user?.result.name}</Typography>
            <Button variant="contained" className={classes.logout} color="secondary" onClick={}>Logout</Button> {/* need to write logout logic still */}
          </div>
        ) : (
